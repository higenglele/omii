...
> 本文档描述建议采用的技术方案，不代表相关代码已经实现或验证。需求文档中仍待确认的“恢复默认窗口大小”“余额美元符号”和“API Key 为空时的界面状态”，不会在本方案中擅自定案。
...
工程部署目标 `26.5` 与 README 中的系统要求描述不一致。本文只记录工程中的实际值，不判断哪个值正确。进入实现前应确认正式支持的最低 macOS 版本，并统一工程设置与文档。
...
工程当前设置为 macOS `26.5`，README 描述为 macOS `15.0+`。滚动监听和窗口 API 的可用方案会受最低系统版本影响，因此正式编码前必须先确定最低支持版本。

## 11. 推荐实施顺序

1. 确认最低 macOS 版本与剩余产品待确认项。
...
正式引入 `swift-markdown` 时应锁定当时经过验证的稳定版本或提交，不应直接长期跟随 `main` 分支。本文没有指定版本号，因为尚未实际接入、编译和兼容性验证。
