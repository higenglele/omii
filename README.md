...
- **Markdown 渲染**: 支持标题、粗体、斜体、删除线、行内代码、链接、有序/无序列表、引用、分隔线和围栏代码块；流式回复期间持续渲染，未闭合标记暂时显示为普通文本
- **代码块复制**: 每个代码块拥有独立的复制按钮，只复制代码正文
- **用户滚动优先**: 阅读旧内容时自动滚动暂停，回到底部后恢复跟随
- **窗口缩放与记忆**: AI 回复页可拖拽缩放（最小 380×360 pt），尺寸跨会话和重启保持
- **图钉固定**: AI 回复页右上角固定图标，固定后窗口保持在普通应用窗口上方
- **多结果窗口**: 支持多个独立结果窗口，自动错开排列，各自独立回复、移动、缩放和固定
- **OpenRouter 余额**: 位于设置 → 通用 → API 配置，自动刷新、显示美元余额和连接状态
- **拖拽排序**: 使用直观的拖拽调整提示词顺序
...
4. 检查网络连通性：在终端运行 `curl -I <你的 API 基础 URL>`
5. 如果使用代理，检查代理配置是否正确
6. 查看应用日志（控制台 → Flick）获取详细的网络错误信息
